...
$(document).ready(function () {
    ...
    $('#submit-form').submit(function (e) {
        e.preventDefault();
        let error = false;
        if (typeof isValidListing !== 'undefined' && !empty(isValidListing)) {
            error = validListingForm();
        }
        if (typeof (formValid) !== 'undefined' && !empty(formValid) && !$('#submit-form').valid()) {
            ...
            if (!empty(error)) {
                ...
                if (isValidListing == true && typeof (tinyMCE.activeEditor.getContent()) !== 'undefined' && !empty(tinyMCE.activeEditor.getContent())) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success === true) {
                            ...
                            $('#submit-form')[0].reset();
                            if (typeof (isGotoIndex) !== 'undefined' && !empty(isGotoIndex)) {
                                if (!empty(response.data)) {
                                    if (!empty(response.data.sub_unique_id)) {
                                        window.location.href = indexRoute + '/' + response.data.unique_id + '/' + response.data.sub_unique_id;
                                    } else if (!empty(response.data.unique_id)) {
                                        window.location.href = indexRoute + '/' + response.data.unique_id;
                                    }
                                } else {
...
function renderMagic() {
    ...
    renderAction = function renderAction(keyValue) {
        ...
        html += '<td>';
        let route = '';
        if (typeof (isAddBtn) !== 'undefined' && !empty(isAddBtn)) {
            route = addBtnRoute + '/' + keyValue.unique_id;
            html += '<a href="' + route + '" class="btn btn-primary btn-sm">' + addBtnText + '</a>'
        }
        if (typeof (AddListingBtn) !== 'undefined' && !empty(AddListingBtn)) {
            route = addListingBtnRoute + '/' + keyValue.unique_id;
            if (typeof (isCategory) !== 'undefined' && !empty(isCategory)) {
                route = addListingBtnRoute + '/' + keyValue.unique_id + '/0';
            } else if (typeof (isSubCategory) !== 'undefined' && !empty(isSubCategory)) {
                route = addListingBtnRoute + '/' + keyValue.category_unique_id + '/' + keyValue.unique_id;
            }
            html += '<a href="' + route + '" class="btn btn-primary btn-sm m-1">' + addListingBtnText + '</a>'
        }
        html += '<a href="' + editRoute + '/' + id + '/edit" id="edit_' + id + '"  class="m-1 text-primary edit-button-group"><i class="fa-solid fa-pen-to-square"></i></a>' +
            '<a href="#"  id="delete_' + id + '" class="m-1 text-danger delete_content"><i class="fa-solid fa-trash"></i></a>\n' +
            '</td>';
    };
